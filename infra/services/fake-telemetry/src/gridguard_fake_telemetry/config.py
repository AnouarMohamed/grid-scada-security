...
class SourceConfig:
    ...
    def from_env(cls) -> SourceConfig:
        return cls(
            ...
            port=_get_port("GRIDGUARD_SOURCE_PORT", 8080),
...
class IngestConfig:
    ...
    def from_env(cls) -> IngestConfig:
        return cls(
            ...
            interval_seconds=_get_positive_float("GRIDGUARD_INGEST_INTERVAL_SECONDS", 2.0),
            request_timeout_seconds=_get_positive_float(
                "GRIDGUARD_REQUEST_TIMEOUT_SECONDS",
                5.0,
            ),
            status_file=Path(
                os.getenv(
                    "GRIDGUARD_INGEST_STATUS_FILE",
                    "/tmp/gridguard-ingestor-ready",  # nosec B108
                )
            ),
        )
